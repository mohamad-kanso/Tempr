...
- [ ] `DriverConnection::ping` → pool `recycle` health check (09-database-engine: idle ping every 30 s, evict broken connections, reconnect with backoff → `Reconnecting`/`Failed` states)
