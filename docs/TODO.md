...
- [ ] Adopt Zed's `gpui-test` agent skill (`.agents/skills/gpui-test/SKILL.md` upstream) as `.claude/skills/gpui-test/`, minus `ZED_HEADLESS`: the pinned gpui rev already ships `#[gpui::test]`, `TestAppContext`, `SEED`/`ITERATIONS`, `PENDING_TRACES=1`. Then add deterministic gpui tests for `MainWindow`, `EditorView`, the palette and keybinding dispatch — all verified only by screenshot or user-run today (`tempr_ui` has 19 plain `#[test]`s on pure helpers). Needs gpui `test-support` as a dev-dependency. First regression target: the `ResultGrid` wheel axes fixed on `fix/todo-scroll-direction` (vertical wheel moves rows only, shift-wheel moves the column container only, both with the pointer anywhere in the grid) — today that is verified only by the XTest harness described in the 2026-09-11 PROGRESS session row (2026-09-11)
- [ ] Adopt Zed's `gpui-bench` agent skill as `.claude/skills/gpui-bench/`, dropping the macOS xctrace/Instruments sections for `perf`/`samply`: the pinned rev ships `#[gpui::bench]`, `BenchAppContext`, `bench_renderer`, `bench-support`. Use `bench_renderer` headless for the "10% of frames > 20 ms" item above (CPU layout/prepaint/paint/text shaping is the suspect, so Linux's no-GPU-submission limitation is acceptable). Benches must live in a crate that never enables `test-support` — Cargo unifies features per package, so a `tempr_ui` dev-dependency on `test-support` would leak into its bench graph (2026-09-11)
- [ ] Trial Zed's dylint lints against `tempr_ui` via `cargo dylint` pointed at upstream `tooling/lints` (own pinned nightly, separate from our 1.97.1): `blocking_io_on_foreground` enforces the "no blocking the UI thread" hard rule mechanically; `notify_in_render` / `entity_update_in_render` catch render-cycle bugs; `shared_string_from_str_literal` / `owned_string_into_shared` / `map_lookup_then_insert` / `async_block_without_await` are general. Report hit count, then decide on a CI warning-level check per 15-coding-standards "Custom lints via dylint". Zed's `lint-creator` skill itself is not portable (authoring inside Zed's tree); `zed-cherry-pick` is Zed-release-only (2026-09-11)
