...
- [ ] Per-connection setting for the default catalog scope: today `SchemaScope::SearchPath` is the only refresh scope a driver call ever passes; a manual "refresh all schemas" action (`SchemaScope::All`) is planned for a later Phase 3 stage, and a per-connection default (sticking with `SearchPath` vs `All`) should live on the connection config once that lands
...
- [ ] `renumber_second_clause` in `tempr_db_postgres::driver` (used by `schema_fingerprints`) does a plain substring replace on `"$N"` to shift the second UNION clause's bind numbers; correct today (`SchemaScope` binds at most 2 params) but a future scope binding ten or more parameters would collide `"$1"` with the leading digits of `"$10"` — make it digit-boundary safe before that happens
