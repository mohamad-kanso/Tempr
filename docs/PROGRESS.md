...
- **Last completed**: Phase 2 kick-off — `tempr_editor` crate with `Buffer` on `ropey` (byte-offset API, validated atomic batch edits, transactional undo/redo, point↔offset mapping; 9 tests + ignored 10 MB probe: **avg 1.96 µs, worst 37.6 µs** per insert+delete in release) (→ D21; 2026-09-03, branch `feat/ph2-rope-buffer`)
- **Verified**: `cargo fmt --all --check` ✅ · `cargo clippy --workspace --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (103 unit) · `cargo test -p tempr_editor --release -- --ignored` ✅ (10 MB insert+delete avg 1.96 µs, worst 37.6 µs) · `cargo deny check` ✅ (2026-09-03)
- **Next action**: tree-sitter PostgreSQL grammar → `SyntaxTree` on `Buffer` with incremental reparse fed from recorded edits (Phase 2 box 2); grammar crate choice is a new dependency → DECISIONS entry
...
### Phase 2 — Editor
- [x] Rope buffer handles 10 MB documents with sub-millisecond insert/delete *(`tempr_editor::Buffer`; release probe on a 10 MB buffer: avg 1.96 µs, worst 37.6 µs per insert+delete; 9 unit tests; 2026-09-03, → D21)*
