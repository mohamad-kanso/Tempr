...
- **Last completed**: Phase 1 database layer — all 94 tests pass (87 unit + 7 integration against Docker PG) (2026-07-14)
- **Verified**: `cargo fmt --all` ✅ · `cargo clippy --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (87 unit tests) · `cargo test -- --ignored` ✅ (7 integration tests: connect, insert/select, streaming, auth failure, schema refresh, syntax error, events)
