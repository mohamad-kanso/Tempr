...
- **Last completed**: Service lifecycle + pooling — `ConnectionService` pools `Box<dyn DriverConnection>` with `deadpool` (user pool max 8 + dedicated metadata slot, eager warm-up connection; → D19); `ConnectionService`/`QueryService`/`SchemaService` implement `Service` and are registered in `ServiceRegistry` (stop drains pools / cancels runs); `SchemaService` refreshes on the metadata slot; `deadpool-postgres` removed (2026-09-03, branch `feat/ph1-service-lifecycle-pool`)
- **Verified**: `cargo fmt --all --check` ✅ · `cargo clippy --workspace --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (87 unit: services 24→29 with the pool/lifecycle tests) · `cargo test -p tempr --test integration -- --ignored` ✅ (10 vs Docker PG 16) · `cargo deny check` ✅ · app run against Docker PG through the pool: Connecting→Connected, startup query QueryStarted→QueryFinished (2026-09-03)
- **Next action**: TLS for the PostgreSQL driver (last open Phase 1 box; needs a TLS connector crate → decision), then workspace open + connection picker replacing `DATABASE_URL`
...
| 2026-09-03 | `ConnectionService` pools via `deadpool` core over the driver trait; separate 1-slot metadata pool for `SchemaService`; `connect` warms one connection; `Service::stop` drains pools, `QueryService::stop` cancels runs | → D19 |
