...
- **Last completed**: PostgreSQL TLS — rustls connector (`tokio-postgres-rustls`, `ring`, platform roots) with libpq `sslmode` semantics on `Connection.tls: TlsMode` (default `prefer`), `?sslmode=` in `DATABASE_URL`, manifest `ConnectionConfig.tls`; cancel reuses the connector (→ D20). **All Phase 1 checklist boxes are now checked.** (2026-09-03, branch `feat/ph1-pg-tls`)
- **Verified**: `cargo fmt --all --check` ✅ · `cargo clippy --workspace --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (94 unit) · `DATABASE_URL=… DATABASE_URL_TLS=… cargo test -p tempr --test integration -- --ignored` ✅ (15: 10 existing + `pg_tls_require_encrypts_the_session`, `pg_tls_prefer_uses_tls_when_the_server_offers_it`, `pg_tls_disable_stays_plaintext_on_a_tls_server`, `pg_tls_verify_full_rejects_a_self_signed_certificate`, `pg_tls_require_fails_against_a_plaintext_only_server`) against Docker PG 16 plain (:55432) and `ssl=on` self-signed (:55433) · `cargo deny check` ✅ · app run with `?sslmode=require` connects and queries; `verify-full` → Failed (2026-09-03)
- **Next action**: Phase 1 exit review — sweep TODO "Now", verify every Phase 1 box once more, update CLAUDE.md "Current phase", then start Phase 2 (Editor) with the rope buffer; workspace open + connection picker remain the first Phase 2-adjacent UI task
