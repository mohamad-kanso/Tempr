...
- [x] Result grid displays streaming rows; smooth scroll for up to 100,000 rows *(streaming: `RowSink` → channel → `uniform_list`, integration test + user screenshot; scroll: `DebugScrollBenchmark` release build 58.8 fps, p95 16.8 ms, 7/603 frames > 20 ms, max 100 ms — vsync-locked; 2026-09-03)*
...
| 2026-09-03 | `CancelQuery` (escape) cancels every active run on the connection (`active_runs()` → `cancel`) | `execute_streaming` yields the run id only on completion; with one connection there is at most one run — return the id eagerly when the connection picker lands |
