...
- **Last completed**: Scroll frame-time probe + cancel — `DebugScrollBenchmark` (ctrl-shift-b) sweeps the grid via `on_next_frame`; **display-on release run at 100,000 rows: 57.2 fps, avg 17.5 ms, p95 23.1 ms, max 59 ms, 61/603 frames > 20 ms** (worst case: full-viewport jump every frame); `CancelQuery` (escape) targets the view-allocated `QueryRunId` via `execute_streaming_with_id` + pending-cancel arming; dev knobs `TEMPR_STARTUP_SQL` / `TEMPR_BENCH_SCROLL=1` (2026-09-03, branch `feat/ph1-grid-perf-cancel`)
- **Verified**: `cargo fmt --all --check` ✅ · `cargo clippy --workspace --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (82 unit) · `cargo test -p tempr --test integration -- --ignored` ✅ (10 vs Docker PG 16) · `cargo deny check` ✅ · scroll bench run by user with display on, window focused, `--release`: 603 frames, 57.2 fps, p95 23.1 ms, 61 dropped; `rendered_rows` advanced 0→99463 (2026-09-03)
- **Next action**: Connection picker in `MainWindow` (workspace connection list) replacing `DATABASE_URL`; core services implement `Service`; deadpool wiring; profile the 10% > 20 ms bench frames (text shaping of a fresh viewport) as time allows
