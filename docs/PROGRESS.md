...
- **Last completed**: Phase 2 kick-off — `tempr_editor` crate with `Buffer` on `ropey` (byte-offset API, validated atomic batch edits, transactional undo/redo, point↔offset mapping; 9 tests + ignored 10 MB probe: **avg 1.73 µs, worst 12.1 µs** per insert+delete in release) (→ D21; 2026-09-03, branch `feat/ph2-rope-buffer`)
- **Verified**: `cargo fmt --all --check` ✅ · `cargo clippy --workspace --all-targets -- -D warnings` ✅ · `cargo test --workspace` ✅ (103 unit) · `cargo test -p tempr_editor --release -- --ignored` ✅ (10 MB insert+delete avg 1.73 µs, worst 12.1 µs) · `cargo deny check` ✅ (2026-09-03)
