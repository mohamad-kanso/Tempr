...
3. Statement boundaries come from the tree: each `statement`/`transaction`/`block` child of `program`, with a following `;` sibling folded in; comments are skipped; dollar-quoted bodies and string literals are opaque to the boundary. This is the statement detector — no separate hand-written scanner.
4. Syntax highlighting uses the grammar's bundled `highlights.scm`, compiled once per process.
