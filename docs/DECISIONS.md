...
| D29 | 2026-09-11 | Nested scroll containers set `restrict_scroll_to_axis` so a one-axis wrapper never hijacks the other axis' wheel; `ResultGrid` container + rows are the reference (11-gpui.md hard rule 6) | Claude (`fix/todo-scroll-direction`) |
...
## D29 — Nested scroll containers scroll their own axis only (2026-09-11)

**By**: Claude (branch `fix/todo-scroll-direction`), fixing the user-reported wheel bug.
**Decision**: Any element that scrolls in one axis and contains or sits inside another scrollable sets gpui's `restrict_scroll_to_axis` (11-gpui.md hard rule 6). `ResultGrid` is the reference: the `overflow_x_scroll` column container sets it through the builder; the `uniform_list` rows set it through `interactivity().base_style`, because `UniformList` does not implement `StatefulInteractiveElement`, where the builder lives.
**Why**: gpui's `Interactivity::paint_scroll_listener` maps a wheel delta onto the *other* axis whenever an element scrolls in one axis alone and the flag is unset. A horizontal-only wrapper therefore consumes every vertical wheel tick as horizontal scroll, and a vertical-only list consumes shift-wheel as vertical scroll — the grid slid sideways on every wheel tick and shift changed nothing. Zed's own `data_table` example opts into the same flag for the same reason. Restructuring the grid so one element scrolls both axes was rejected: the header must stay in the horizontal container while the rows virtualize vertically, and `restrict_scroll_to_axis` is the upstream-sanctioned opt-out.
**Consequences**: every future scrollable that nests (sidebar trees inside panels, editor inside a split, the palette list) must carry the flag or it will hijack the outer container's wheel; reviewers check for it. A one-axis element that stands alone may keep gpui's default, which is a convenience (a horizontal wheel scrolls a vertical-only list). The grid's column area is also `min_w_full` so the rows' hitbox spans the viewport and the wheel works beside a narrow result set. Regression coverage waits on the gpui-test adoption item in TODO.
