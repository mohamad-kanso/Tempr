...
**Consequences**: Borrow sites take a `PooledConnection` by value and return `Result<R, DriverError>`; the connection returns to the pool on drop. Idle health checks (`recycle` ping) and reconnection/backoff need a `DriverConnection::ping` — tracked in TODO. Pool sizing per connection will come from the workspace connection config (`pool_max_size` in 09-database-engine's `ConnectionConfig`); today `PoolConfig` is service-wide. Broken connections are not yet evicted on error (needs the ping to detect).
